...
const AdminExperience = () => {
  ...
  const form = useForm<ExperienceFormData>({
    ...
    defaultValues: { company: '', position: '', start_date: '', end_date: '', is_current: false, description: '' },
  });
  ...
  const openEditDialog = (item: Experience) => {
    setEditingItem(item);
    form.reset({ company: item.company, position: item.position, start_date: item.start_date || '', end_date: item.end_date || '', is_current: item.is_current, description: item.description || '' });
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild><Button onClick={openNewDialog}><Plus className="mr-2 h-4 w-4" />Add Experience</Button></DialogTrigger>
            <DialogContent className="max-w-lg">
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  <FormField control={form.control} name="company" render={({ field }) => (<FormItem><FormLabel>Company</FormLabel><FormControl><Input placeholder="Company name" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  <FormField control={form.control} name="position" render={({ field }) => (<FormItem><FormLabel>Position</FormLabel><FormControl><Input placeholder="Your role" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <FormField control={form.control} name="end_date" render={({ field }) => (<FormItem><FormLabel>End Date</FormLabel><FormControl><Input type="date" {...field} disabled={form.watch('is_current')} /></FormControl><FormMessage /></FormItem>)} />
                  </div>
                  <FormField control={form.control} name="is_current" render={({ field }) => (<FormItem className="flex items-center gap-2"><FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl><FormLabel className="!mt-0">I currently work here</FormLabel></FormItem>)} />
        ...
        {experience.length > 0 ? (
                ...
                <CardContent className="flex items-center gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {item.start_date && format(new Date(item.start_date), 'MMM yyyy')} - {item.is_current ? 'Present' : item.end_date && format(new Date(item.end_date), 'MMM yyyy')}
                    </p>
                  </div>
          ...
          <Card><CardContent className="flex flex-col items-center justify-center py-12"><p className="text-muted-foreground mb-4">No experience added yet</p><Button onClick={openNewDialog}><Plus className="mr-2 h-4 w-4" />Add Your First Experience</Button></CardContent></Card>
        )}
      </div>
    </AdminLayout>
  );
